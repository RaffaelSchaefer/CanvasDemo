...
export function blendedRGBNoise (ctx: any, width: number, height: number, resolution: number, alpha: number) {
    ...
    for (let x = 0; x <= width/resolution; x++) {
        for (let y = 0; y <= height/resolution; y++) {
            ...
            if (x%2 && y%2) {
                ...
                let blendedColor: colorRGBA = new colorRGBA((randomColor.r+lastRandomColor.r)/2, (randomColor.g+lastRandomColor.g)/2, (randomColor.b+lastRandomColor.b)/2, alpha);
